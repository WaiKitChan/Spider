...
PDEAL CreateDeal(HDC hdc) {
	...
	deal->rc.bottom=G_HEIGHT-G_PADDING;
...
PSTACK CreateStack(HDC hdc) {
	...
	stack->rc.bottom=G_HEIGHT-G_PADDING;
	...
	HBITMAP hbmV=CreateCompatibleBitmap(hdc,G_WIDTH,G_HEIGHT);
...
void Victory(HWND hwnd,PSTACK stack,PTEXTURE texture) {
	BitBlt(stack->hdcVictory,0,0,G_WIDTH,G_HEIGHT,texture->bg,0,0,SRCCOPY);
...
BOOL Physics(WORD card,HWND hwnd,PSTACK stack,PTEXTURE texture) {
	...
	if(stack->pt[card].y>G_HEIGHT-CARD_HEIGHT/2){
		stack->pt[card].y=G_HEIGHT-CARD_HEIGHT/2;
